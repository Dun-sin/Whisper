...
const Chat = () => {
	...
	const handleSubmit = async (e) => {
		...
		if (inputRef.current) {
			...
			setMessage(''); // Reset the state of the message to an empty string
	...
	return (
		<div className="w-full md:h-[90%] min-h-[100%] pb-[25px] flex flex-col justify-between gap-6">
			<div className="max-h-[67vh]">
				...
				>
					{sortedMessages.map(({ senderId: sender, id, message, time, status }) => {
						...
						return (
							...
							>
								<div className="message">
									...
									<div className={`status ${status === 'failed' ? 'text-red-600' : 'text-white'}`}>
										<MessageStatus
											time={getTime(time)}
											status={status ?? 'sent'}
