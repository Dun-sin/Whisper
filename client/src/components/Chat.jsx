	...
	return (
		<div className="w-full md:h-[90%] min-h-[100%] pb-[25px] flex flex-col justify-between gap-6">
			<div className="max-h-[67vh]">
				...
				>
					{sortedMessages.map(({ senderId: sender, id, message, time, status, replyTo }) => {
						...
						return (
							<div
								...
								className={createClassesFromArray(
									...
									currentReplyMessageId === id && 'bg-[#FF9F1C]/25 border-[#FF9F1C]',
							...
							>
								...
								<div
									className={createClassesFromArray(
										...
										sender.toString() === senderId.toString() ? 'me' : 'other'
									)}
								>
									<div className="message">
										...
										>
											...
											{sender.toString() === senderId.toString() && status !== 'pending' && (
												...
												>
													...
													<Dropdown.Item onClick={() => startReply(id)}>Reply</Dropdown.Item>
										...
										<div
											className={`status ${status === 'failed' ? 'text-red-600' : 'text-white'}`}
										>
											<MessageStatus
												time={getTime(time)}
												status={status ?? 'sent'}
