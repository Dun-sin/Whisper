...
const Status = ({ id, isSender, doSend, time, isEdited, oldMessages }) => {
	const { app } = useApp();
	...
	return (
		<div className={`flex gap-2 items-center ${isSender ? 'flex-row' : 'flex-row-reverse'}`}>
			<div className={`text-[12px] ${status === 'failed' ? 'text-red-600' : 'text-white'}`}>
				<MessageStatus
					time={getTime(time)}
					status={status ?? 'sent'}
